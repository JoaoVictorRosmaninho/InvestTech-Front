...
const FundReports = () => {
  ...
  const columns = React.useMemo(
   ...
   React.useEffect(() => {
    ...
    axios
      .get(`http://localhost:3001//portifolios/saldo/${id}/${date}`)
        .then((resp) => { setYesterday(resp.data); })
        .catch((err) => { console.log(err); });
    date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+ (today.getDate());
    axios
      .get(`http://localhost:3001/portifolios/pl/${id}/${date}`)
    ...
    return (
      <Container className="mt-4" fluid="sm">
        ...
        <Row>
        <Col><strong>Data: {today.getDate() + '/' + (today.getMonth() + 1) + '/' + today.getFullYear()}</strong></Col> 
        </Row>
        <Row className="mt-4">
          <Col>
            <Chart
